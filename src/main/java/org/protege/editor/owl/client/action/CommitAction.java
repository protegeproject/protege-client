...
public class CommitAction extends ProtegeOWLAction {
    ...
    public void actionPerformed(ActionEvent arg0) {
        try {
            ...
            final String commitComment = JOptionPane.showInputDialog(owner, "Commit comment: ", "Commit",
                    JOptionPane.PLAIN_MESSAGE);
            if (vont == null) {
                JOptionPane.showMessageDialog(owner,
                        "Commit ignored because the ontology is not associated with a server");
                return;
            }
            ...
            connectionManager.getSingleThreadExecutorService()
                    .submit(new DoCommit(client, vont, (!commitComment.isEmpty() ? commitComment : "")));
        }
        catch (UserDeclinedAuthenticationException udae) {
            ; // ignore this because the user knows that he didn't want to
              // authenticate
        }
        catch (Exception e) {
            handleError(e);
